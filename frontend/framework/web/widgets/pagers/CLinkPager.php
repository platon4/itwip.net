...
class CLinkPager extends CBasePager
{
	...
	{	
		...
		$buttons[] = CHtml::openTag("span", array('class' => 'page_nav_pageList'));

			if( $this->getPageCount() <= 10 ) {
				
				for($j = 1; $j <= $this->getPageCount(); $j ++) {
					
					if( $j != $currentPage ) {
						$buttons[]=$this->createPageButton($j,$j,$this->internalPageCssClass,false,$j==$currentPage);
					} else
						$buttons[]= "<span>$j</span> ";
				}
			} else {
				...
				if( $start >= 2 ) {
					$buttons[] = $this->createPageButton(1,1,$this->internalPageCssClass,false,false)." <span class=\"nav_ext\">...</span> ";
				}
				...
				if( $currentPage != $this->getPageCount() ) {
					$buttons[] = $nav_prefix . $this->createPageButton($this->getPageCount(),$this->getPageCount(),$this->internalPageCssClass,false,$this->getPageCount()==$currentPage);
				} else
					$buttons[] = "<span>" . $this->getPageCount() . "</span> ";
			
			}
			
			$buttons[] = CHtml::closeTag("span");
		
		if(($page=$currentPage+1)>=$pageCount)
			$page=$pageCount;		
		$buttons[]=$this->createPageButton($this->nextPageLabel,$page,$this->nextPageCssClass,$currentPage>=$pageCount,false);
	...
	protected function createPageButton($label,$page,$class,$hidden,$selected)
	{
		...
		return CHtml::link($label, 'javascript:;', array('onclick' => $this->ajax_fnct.'(\'' . $page . '\'); return false;', 'class' => $class));
	}
