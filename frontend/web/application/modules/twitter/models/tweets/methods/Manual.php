...
class Manual extends \FormModel
{
	...
	{
		return $this->sDate === NULL ? date('Y-m-d H:i:s') : $this->sDate;
	}
	...
	public function create()
	{
		...
		try {
			 ...
			 * Создаем заказ
			 */
			$db->createCommand("INSERT INTO {{twitter_orders}} (owner_id,type_order,order_hash,order_cost,return_amount,start_date,create_date,status,all_taks,payment_type,_params) VALUES (:owner,:type_order,:order_hash,:order_cost,:return_amount,:start_date,:create_date,:status,:all_taks,:payment,:_params)")
				->execute([
					':owner' => Yii::app()->user->id,
					':type_order' => 'manual',
					':order_hash' => $this->_tid,
					':order_cost' => $this->getSum(),
					':return_amount' => $this->getExtractSum(),
					':start_date' => $this->orderStartDate(),
