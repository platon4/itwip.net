...
class Accounts
{
		...
		if($this->_load === NULL) {
			if(is_array($this->accounts)) {
				...
				if(is_array($ids) && $ids !== array()) {
					$accounts = Yii::app()->db->createCommand("SELECT tw.*,st.working_in,st.fast_posting,st.allow_retweet,st.allow_following,st._subjects,st._gender,st._age,st._price,st._timeout,st._stop,st._filter,st._allow_bonus_pay FROM {{tw_accounts}} tw INNER JOIN {{tw_accounts_settings}} st ON tw.id=st.tid WHERE tw.id IN('" . implode("', '", $ids) . "')")->queryAll();
					$cache    = [];
				...
				if($cache !== false && !\CHelper::isEmpty($cache)) {
					...
					$row                          = Yii::app()->db->createCommand("SELECT tw.*,st.working_in,st.fast_posting,st.allow_retweet,st.allow_following,st._subjects,st._gender,st._age,st._price,st._timeout,st._stop,st._filter,st._allow_bonus_pay FROM {{tw_accounts}} tw INNER JOIN {{tw_accounts_settings}} st ON tw.id=st.tid WHERE tw.id=:id")->queryRow(true, [':id' => $this->accounts]);
	...
	{
		if($this->_errors === array()) return 'System Error';
