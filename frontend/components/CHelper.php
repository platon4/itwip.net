...
class CHelper
{
    ...
    public static function _getURL($url, $method = "GET", $data = array(), $headers = array(), $cookie = false)
    {
        ...
        if($cookie) {
            if(!is_dir(Yii::app()->getBasePath() . '/cookies')) {
                mkdir(Yii::app()->getBasePath() . '/cookies');
                @chmod(Yii::app()->getBasePath() . '/cookies', 0777);
            }

            curl_setopt($curl, CURLOPT_COOKIEJAR, Yii::app()->getBasePath() . '/cookies/cookie_' . md5(self::_getIP() . self::_getDomen($url)) . '.txt'); //сохранить куки в файл
            curl_setopt($curl, CURLOPT_COOKIEFILE, Yii::app()->getBasePath() . '/cookies/cookie_' . md5(self::_getIP() . self::_getDomen($url)) . '.txt'); //считать куки из файла
        }
