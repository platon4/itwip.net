	...
	public static function _getLogNote($type, $for, $operationData, $notice = '')
	{
		$data = array(
			...
			0 => array(
				...
				4 => Yii::t('financeModule.index', '_checking_tweets_delete', array(
						'{account}' => $notice)),
				5 => Yii::t('financeModule.index', '_checking_tweets_deletion_successfully', array(
						'{account}' => $notice))
			),
			//Расход
			1 => array(
				0 => Yii::t('financeModule.index', '_providing_collateral_order', array(
						'{id}' => $operationData)),
				1 => Yii::t('financeModule.index', '_out_money'),
				2 => Yii::t('financeModule.index', '_order_executed_successful_completion', array(
						'{id}' => $operationData)),
				3 => Yii::t('financeModule.index', '_checking_tweets_delete_unsuccessfully', array(
						'{account}' => $notice)),
				4 => Yii::t('financeModule.index', '_shop_ref_buy', array('{ref}' => $notice)),
			),
			//Возврат
			2 => array(
				0 => Yii::t('financeModule.index', '_removalOrder_return_unspent_funds', array(
						'{id}' => $operationData)), //возврат за заказ
				1 => Yii::t('financeModule.index', 'twitter_money_return_tweet', array(
