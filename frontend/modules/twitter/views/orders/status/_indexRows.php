        ...
        <?php foreach($model->m->getRows() as $row) { ?>
            ...
            <tr id="order_<?php echo $row['id']; ?>" data-amount="<?php echo $row['return_amount']; ?>" data-atype="<?php echo $row['payment_type']; ?>">
                ...
                <td class="progress_order">
                    <div data-percent="<?= $model->m->getCompleteProcent($row['completed_taks'], $row['all_taks']); ?>%" class="progress progress-success progress-striped">
                        <div style="width: <?= $model->m->getCompleteProcent($row['completed_taks'], $row['all_taks']); ?>%;" class="bar"></div>
                    </div>
...
<?php } else { ?>
    <div style="text-align: center; padding: 10px;">У вас не одного заказа, хотите
        <a href="/twitter/tweets/collection">создать</a> ?
    </div>
<?php } ?>