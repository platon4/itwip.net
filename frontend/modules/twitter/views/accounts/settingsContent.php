...
<?php if($model->updateProcess()) { ?>
    <div id="loadFade" class="load_sec">
        <div id="fade-content" style="padding-top: 100px; margin: 0 10px;">
            <div>
                Происходит сбор данных, это может занять некоторое время.<br>По завершению операции Вы сможете настроить аккаунт для работы в системе.<br><br>
            </div>
            <div><img src="/i/loading_11.gif" alt=""/></div>
        </div>
    </div>
    <script>
        Accounts.data.update('<?= $model->get('id'); ?>');
    </script>
<?php } ?>
</div>