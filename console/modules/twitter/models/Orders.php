...
class Orders extends Model
{
    ...
    public function createOrders()
    {
        if($this->isRun() === false) {
            ...
            $orders = $query
                ...
                ->from('{{%twitter_orders}}')
                ->where(['and', 'status=:status', 'process_date<=:date', 'is_process=0', 'owner_id=2'], [':status' => 1, ':date' => date('Y-m-d')])
                ->orderBy(['id' => SORT_ASC])
                ->limit(10)
                ->all();
    ...
    {
        if($key == 'columns')
            return ['order_id', 'order_hash', 'sbuorder_id', 'orderType', 'tweet_hash', 'url_hash', 'process_time', 'payment_type', 'params', 'daemon'];
        else
            return $this->_orders;
    }
