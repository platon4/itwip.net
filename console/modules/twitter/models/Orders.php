...
class Orders extends Model
{
    ...
    public function createOrders()
    {
        if($this->isRun() === false) {
            ...
            $orders = $query
                ...
                ->from('{{%twitter_orders}}')
                ->where(['and', 'status=:status', 'process_date<=:date', 'is_process=0', 'bloger_amount>0'], [':status' => 1, ':date' => date('Y-m-d')])
                ->orderBy(['id' => SORT_ASC])
                ->limit(10)
                ->all();
    ...
    {
        if($key == 'columns')
            return ['order_id', 'order_hash', 'sbuorder_id', 'orderType', 'tweet_hash', 'domen', 'tw_account', 'process_time', 'payment_type', 'params', 'daemon', 'bloger_amount', 'adv_amount'];
        else
            return $this->_orders;
    }
